  ...
  {error && <ErrorMsg theme={{ darkMode }}>{error}</ErrorMsg>}
        <form onSubmit={handleLogin}>
          ...
          <Input theme={{ darkMode }}
            ...
            required
          />
          <Button theme={{ darkMode }} type="submit" disabled={loading}>{loading ? 'Signing in...' : 'Sign In'}</Button>
        </form>
        <div style={{ width: '90%', maxWidth: 320, margin: '16px auto 0 auto', textAlign: 'center' }}>
          <span style={{ color: darkMode ? '#bbb' : '#222', fontSize: '0.98rem' }}>
            ...
            <span style={{ color: darkMode ? '#fff' : '#181818', textDecoration: 'underline', cursor: 'pointer' }} onClick={() => navigate('/register')}>
              Sign Up
            </span>
          </span>
        </div>
